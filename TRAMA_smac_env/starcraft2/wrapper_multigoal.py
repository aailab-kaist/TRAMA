from smacv2.env.starcraft2.distributions_multigoal import get_distribution
from smacv2.env.starcraft2.starcraft2 import StarCraft2Env, CannotResetException
from smacv2.env import MultiAgentEnv


class StarCraftCapabilityEnvWrapper_multigoal(MultiAgentEnv):
    ...
    def _parse_distribution_config(self):
        for env_key, config in self.distribution_config.items():
            if env_key == "n_units" or env_key == "n_enemies" or env_key == "unit_pos_mix" or \
                env_key == "n_combination" or env_key == "dist_ratio" or env_key == "unit_team_type":
                continue
            config["env_key"] = env_key
            # add n_units key
            config["n_units"] = self.distribution_config["n_units"]
            config["n_enemies"] = self.distribution_config["n_enemies"]
            config["n_combination"] = self.distribution_config["n_combination"]
            config["dist_ratio"] = self.distribution_config["dist_ratio"]
            config["unit_team_type"] = self.distribution_config["unit_team_type"]
            config["unit_pos_mix"] = self.distribution_config["unit_pos_mix"]
    ...
    def reset(self):
        try:
            ...
            for distribution in self.env_key_to_distribution_map.values():
                reset_config = {**reset_config, **distribution.generate()} # id value is not used

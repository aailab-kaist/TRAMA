...
class WeightedTeamsDistribution(Distribution):
    def __init__(self, config):
        ...
        self.env_key = config["env_key"]
        self.unit_team_type = config["unit_team_type"]
        self.unit_pos_mix = config["unit_pos_mix"]
        
        # multi-goal-defition
        self.n_combination = config["n_combination"]
        
        self.unit_list =[]
        if self.n_combination ==4:            
            self.unit_list.append( [ self.units[0],self.units[0],self.units[1],self.units[1],self.units[2] ] ) # 1c2s2z
            self.unit_list.append( [ self.units[0],self.units[0],self.units[0],self.units[1],self.units[1] ] ) # 3s2z
            self.unit_list.append( [ self.units[1],self.units[1],self.units[1],self.units[2],self.units[2] ] ) # 2c3z
            self.unit_list.append( [ self.units[0],self.units[0],self.units[0],self.units[2],self.units[2] ] ) # 2c3s     
        else:
            self.unit_list.append( [ self.units[0],self.units[0],self.units[0],self.units[1],self.units[1] ] ) # 3s2z
            self.unit_list.append( [ self.units[1],self.units[1],self.units[1],self.units[2],self.units[2] ] ) # 2c3z
            self.unit_list.append( [ self.units[0],self.units[0],self.units[0],self.units[2],self.units[2] ] ) # 2c3s     

    def _gen_team_default(self, n_units: int, use_exceptions: bool):
        team = []
        while not team or (
    ...
    def _gen_team(self, n_units: int, use_exceptions: bool):
        ...
        while not team or (
            ...
            #idx_selected = int(self.n_combination*self.rng.random())
            if self.unit_team_type == -1:
                idx_selected = int(self.rng.choice(np.arange(self.n_combination), size=1, p=self.weights))
            else:
                idx_selected = self.unit_team_type
            team = list( self.unit_list[idx_selected])
            
            # team = list(
            #     self.rng.choice(self.units, size=(n_units,), p=self.weights)
            # )
            shuffle(team) # shuffle the generated order
        return team, idx_selected

    def generate(self) -> Dict[str, Dict[str, Any]]:
        team, idx_selected = self._gen_team(self.n_units, use_exceptions=True) # index output
...
class ReflectPositionDistribution(Distribution):
    ...
    def __init__(self, config):
        ...
        self.map_x = config["map_x"] # 32
        self.map_y = config["map_y"] # 32
        config_copy = deepcopy(config)
        config_copy["env_key"] = "ally_start_positions"
        #config_copy["lower_bound"] = (0, 0)
        config_copy["lower_bound"] = (self.map_x/2-self.map_x/2*self.dist_ratio, 
                                      self.map_y/2-self.map_y/2*self.dist_ratio)
        ...
        config_copy["upper_bound"] = (self.map_x / 2 - 1, 
                                      self.map_y/2+self.map_y/2*self.dist_ratio)
    ...
    def generate(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "ally_start_positions": {"item": ally_positions, "id": 0,"map_class": 1},
...
class SurroundedPositionDistribution(Distribution):
    ...
    def generate(self) -> Dict[str, Dict[str, Any]]:
        ...
        diagonal_to_point_map = {
            0: np.array([self.map_x/2-self.map_x/2*self.dist_ratio, self.map_y/2-self.map_y/2*self.dist_ratio]),
            1: np.array([self.map_x/2-self.map_x/2*self.dist_ratio, self.map_y/2+self.map_y/2*self.dist_ratio]),
            2: np.array([self.map_x/2+self.map_x/2*self.dist_ratio, self.map_y/2+self.map_y/2*self.dist_ratio]),
            3: np.array([self.map_x/2+self.map_x/2*self.dist_ratio, self.map_y/2-self.map_y/2*self.dist_ratio]),
        }
        unit_index = 0
        ...
        return {
            "ally_start_positions": {"item": ally_position, "id": 0,"map_class": 0},

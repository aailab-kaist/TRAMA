...
- The results in SMAC (https://arxiv.org/abs/1902.04043) and our paper use SC2.4.6.2.69232 not SC2.4.10.
```
...
```shell
python3 src/main.py --config=vqmix --env-config=sc2 with env_args.map_name=3m
```
...
```shell
bash run.sh $GPU python3 src/main.py --config=vqmix --env-config=sc2 with env_args.map_name=3m
```
